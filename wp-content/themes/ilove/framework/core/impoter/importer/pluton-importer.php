...
class Ilove_Theme_Importer {
    ...
    public function add_admin() {
        ...
        add_submenu_page('themes.php', "Import Demo Data", "Import Demo Data", 'switch_themes', 'ilove_demo_installer', array($this, 'demo_installer'));
    ...
    public function set_demo_theme_options( $file ) {
		...
		if ( ! file_exists( $file ) ) {
			wp_die(
				__( 'Theme options Import file could not be found. Please try again.', 'ilove' ),
				'',
		...
		if ( empty( $data ) || ! is_array( $data ) ) {
			wp_die(
				__( 'Theme options import data could not be read. Please try a different file.', 'ilove' ),
				'',
				array( 'back_link' => true )
			);
		}
		...
		// Hook before import
		$data = apply_filters( 'ilove_theme_import_theme_options', $data );
    ...
    function available_widgets() {
    	...
    	return apply_filters( 'ilove_theme_import_widget_available_widgets', $available_widgets );
    ...
    function process_widget_import_file( $file ) {
    	...
    	if ( ! file_exists( $file ) ) {
    		wp_die(
    			__( 'Widget Import file could not be found. Please try again.', 'ilove' ),
    			'',
    			array( 'back_link' => true )
    		);
    	}
    ...
    public function import_widgets( $data ) {
    	...
    	if ( empty( $data ) || ! is_object( $data ) ) {
    		wp_die(
    			__( 'Widget import data could not be read. Please try a different file.', 'ilove' ),
    			'',
    			array( 'back_link' => true )
    		);
    	}
    	...
    	// Hook before import
    	$data = apply_filters( 'ilove_theme_import_widget_data', $data );
    	...
    	foreach ( $data as $sidebar_id => $widgets ) {
    		...
    		if ( isset( $wp_registered_sidebars[$sidebar_id] ) ) {
    			...
    			$sidebar_message = __( 'Sidebar does not exist in theme (using Inactive)', 'ilove' );
    		}
    		...
    		foreach ( $widgets as $widget_instance_id => $widget ) {
    			...
    			if ( ! $fail && ! isset( $available_widgets[$id_base] ) ) {
    				...
    				$widget_message = __( 'Site does not support widget', 'ilove' ); // explain why widget not imported
    			}

    			// Filter to modify settings before import
    			// Do before identical check because changes may make it identical to end result (such as URL replacements)
    			$widget = apply_filters( 'ilove_theme_import_widget_settings', $widget );
    			...
    			if ( ! $fail && isset( $widget_instances[$id_base] ) ) {
    				...
    				foreach ( $single_widget_instances as $check_id => $check_widget ) {
    					...
    					if ( in_array( "$id_base-$check_id", $sidebar_widgets ) && (array) $widget == $check_widget ) {
    						...
    						$widget_message = __( 'Widget already exists', 'ilove' ); // explain why widget not imported
    			...
    			if ( ! $fail ) {
    				...
    				if ( $sidebar_available ) {
    					...
    					$widget_message = __( 'Imported', 'ilove' );
    				} else {
    					$widget_message_type = 'warning';
    					$widget_message = __( 'Imported to Inactive', 'ilove' );
    				}
    			...
    			$results[$sidebar_id]['widgets'][$widget_instance_id]['title'] = $widget->title ? $widget->title : __( 'No Title', 'ilove' ); // show "No Title" if widget instance is untitled
    	...
    	// Hook after import
    	do_action( 'ilove_theme_import_widget_after_import' );

    	// Return results
    	return apply_filters( 'ilove_theme_import_widget_results', $results );
